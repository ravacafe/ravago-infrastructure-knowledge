...
<#
.SYNOPSIS
    Disables all AMBA suppression alert processing rules across all Ravago subscriptions.
...
.DESCRIPTION
    ...
    and disables it via the Azure REST API.
...
.EXAMPLE
    .\Disable-AMBAAlertProcessingRules.ps1
#>
...
$targetState   = $false   # false = Disabled
...
Write-Host "  DISABLING AMBA Alert Processing Rules — $total subscriptions" -ForegroundColor Cyan
Write-Host "════════════════════════════════════════════════════════════════" -ForegroundColor Cyan
Write-Host ""
...
foreach ($subName in $subscriptions.Keys) {
    ...
    try {
        ...
        if ($currentState -eq $targetState) {
            Write-Host "     ℹ  Already disabled — no action needed." -ForegroundColor DarkGray
            $disabled++
            continue
        }
...
Write-Host "  Disabled (or already disabled) : $disabled" -ForegroundColor Green
